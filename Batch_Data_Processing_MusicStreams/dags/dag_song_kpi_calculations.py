...
def list_s3_files(prefix, bucket = BUCKET_NAME):
    ...
    try:
        ...
        raise Exception(f"Failed to list files with prefix {prefix} from s3: {files}")
...
def validate_datasets():
    ...
    try:
        ...
        if not missing_columns:
            ...
            logging.info("All required columns are present in songs")
        else:
...
######## =================== END: TASK - VALIDATION ASSERTS =============== #####

######## =================== START: TASK - CHECK VALIDATION ASSERTS =============== #####
def branch_task(ti): # ti - task instance while airflow executes a python callable, it can inject runtime context objects into a function
    validation_results = ti.xcom_pull(task_ids = 'validate_datasets') # passing the task id
...
######## =================== END: TASK - CHECK VALIDATION ASSERTS =============== #####
...
######## =================== END: TASK - CALCULATION ASSERTS =============== #####



######## =================== START: TASK - MOVE PROCESSED FILES ASSERTS =============== #####
...
######## =================== START: END - MOVE PROCESSED FILES ASSERTS =============== #####
...
with DAG('data_validation_and_kpi_computation', default_args = default_args, schedule = '@daily') as dag:
    validate_datasets = PythonOperator(
        task_id = 'validate_datasets', # Task ID we are declaring over here
        python_callable = validate_datasets # This is basically calling the function 
    )

    # Checks the validation datasets dict if everything is True to routes to a path, else exits
